...
class Metric(Enum):
    ...
    DEGREE_TO_FREQUENCY_RATIO = 0.0  # Uses d(w)/f(w) as the metric
    WORD_DEGREE = 1.0  # Uses d(w) alone as the metric
    WORD_FREQUENCY = 2.0  # Uses f(w) alone as the metric
...
class CustomRake:
    ...
    def extract_keywords_from_text(self, text: str):
        ...
        count = self.extract_keywords_from_sentences(sentences)
        return count
    ...
    def extract_keywords_from_sentences(self, sentences: List[Sentence]):
        ...
        """
        phrase_list, count = self._generate_phrases(sentences)
    ...
    def _generate_phrases(self, sentences: List[Sentence]) -> List[Phrase]:
        ...
        if not self.include_repeated_phrases:
            ...
            return non_repeated_phrase_list, count

        return phrase_list, count
